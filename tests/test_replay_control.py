...
class TestReplayControl:
    def test_off_for_a_live_camera(self):
        ...
        assert s.take_replay_command() == (False, None, 1.0)

    def test_seekable_file_enables_the_player(self):
        s = DashboardState()
        s.announce_replay(300, gen=0)
        assert s.snapshot()["replay"] == {"total": 300, "cur": 0, "paused": False, "speed": 1.0}

    def test_pause_play(self):
        s = DashboardState()
        s.announce_replay(300, gen=0)
        assert s.replay_control("pause") is True
        assert s.take_replay_command() == (True, None, 1.0)
    ...
    def test_step_moves_from_current_and_pauses(self):
        ...
        paused, seek, _speed = s.take_replay_command()
        assert paused is True and seek == 45

    def test_speed_is_clamped_and_reported(self):
        s = DashboardState()
        s.announce_replay(300, gen=0)
        assert s.replay_control("speed", 2.0) is True
        assert s.take_replay_command()[2] == 2.0
        assert s.snapshot()["replay"]["speed"] == 2.0
        s.replay_control("speed", 99)  # clamped
        assert s.take_replay_command()[2] == 4.0
